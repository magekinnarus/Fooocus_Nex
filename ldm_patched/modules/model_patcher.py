...
class LowVramPatch:
    ...
    def __call__(self, weight):
        ...
        if intermediate_dtype not in [torch.float32, torch.float16, torch.bfloat16]: #intermediate_dtype has to be one that is supported in math ops
            ...
            return ldm_patched.modules.float.stochastic_rounding(calculate_weight(self.patches[self.key], weight.to(intermediate_dtype), self.key, intermediate_dtype=intermediate_dtype), intermediate_dtype, seed=string_to_seed(self.key))
...
class ModelPatcher:
    def __init__(self, model, load_device, offload_device, size=0, weight_inplace_update=False, current_device=None, **kwargs):
        ...
        self.hook_patches: dict[ldm_patched.modules.hooks._HookRef] = {}
        self.hook_patches_backup: dict[ldm_patched.modules.hooks._HookRef] = None
        self.hook_backup: dict[str, tuple[torch.Tensor, torch.device]] = {}
        self.cached_hook_patches: dict[ldm_patched.modules.hooks.HookGroup, dict[str, torch.Tensor]] = {}
        self.current_hooks: Optional[ldm_patched.modules.hooks.HookGroup] = None
        self.forced_hooks: Optional[ldm_patched.modules.hooks.HookGroup] = None  # NOTE: only used for CLIP at this time
        self.is_clip = False
        self.hook_mode = ldm_patched.modules.hooks.EnumHookMode.MaxSpeed
    ...
    def set_hook_mode(self, hook_mode: ldm_patched.modules.hooks.EnumHookMode):
        self.hook_mode = hook_mode

    def prepare_hook_patches_current_keyframe(self, t: torch.Tensor, hook_group: ldm_patched.modules.hooks.HookGroup, model_options: dict[str]):
        curr_t = t[0]
    ...
    def register_all_hook_patches(self, hooks: ldm_patched.modules.hooks.HookGroup, target_dict: dict[str], model_options: dict=None,
                                  registered: ldm_patched.modules.hooks.HookGroup = None):
        self.restore_hook_patches()
        if registered is None:
            registered = ldm_patched.modules.hooks.HookGroup()
        # handle WeightHooks
        weight_hooks_to_register: list[ldm_patched.modules.hooks.WeightHook] = []
        for hook in hooks.get_type(ldm_patched.modules.hooks.EnumHookType.Weight):
    ...
    def add_hook_patches(self, hook: ldm_patched.modules.hooks.WeightHook, patches, strength_patch=1.0, strength_model=1.0):
        with self.use_ejected():
    ...
    def get_combined_hook_patches(self, hooks: ldm_patched.modules.hooks.HookGroup):
    ...
    def apply_hooks(self, hooks: ldm_patched.modules.hooks.HookGroup, transformer_options: dict=None, force_apply=False):
        ...
        if self.current_hooks == hooks and (not force_apply or (not self.is_clip and hooks is None)):
            return ldm_patched.modules.hooks.create_transformer_options_from_hooks(self, hooks, transformer_options)
        self.patch_hooks(hooks=hooks)
        for callback in self.get_all_callbacks(CallbacksMP.ON_APPLY_HOOKS):
            callback(self, hooks)
        return ldm_patched.modules.hooks.create_transformer_options_from_hooks(self, hooks, transformer_options)

    def patch_hooks(self, hooks: ldm_patched.modules.hooks.HookGroup):
        with self.use_ejected():
            if hooks is not None:
                model_sd_keys = list(self.model_state_dict().keys())
                memory_counter = None
                if self.hook_mode == ldm_patched.modules.hooks.EnumHookMode.MaxSpeed:
    ...
    def patch_hook_weight_to_device(self, hooks: ldm_patched.modules.hooks.HookGroup, combined_patches: dict, key: str, original_weights: dict, memory_counter: MemoryCounter):
