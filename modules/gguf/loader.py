...
def gguf_sd_loader(path, handle_prefix="model.diffusion_model.", return_arch=False):
    ...
    if arch_str is None: # stable-diffusion.cpp
        # Compatibility mode for old files requires tools/convert.py which is not ported.
        logging.warning("GGUF: general.architecture missing. Old stable-diffusion.cpp files may not load correctly.")
        arch_str = "sd1" # Default fallback
    elif arch_str in ["pig"]:
        logging.warning("GGUF: 'pig' architecture requires tools/convert.py which is not ported.")
        arch_str = "sd1"
