 ...
 for /f "tokens=*" %%t in ('python -c "import torch; print(torch.__version__)" 2^>nul') do set "TORCH_VER=%%t"
 echo [ OK ] PyTorch %TORCH_VER% with CUDA
 
 REM --- Check xformers ---
 python -c "import xformers" 2>nul
 if errorlevel 1 (
...
if errorlevel 1 (
     ...
     if not errorlevel 1 (
         ...
         echo        Model downloads will use slower fallback methods.
