...
```powershell
.\venv\Scripts\python.exe -m py_compile backend\memory_governor.py backend\resources.py backend\sdxl_runtime_policy.py backend\sdxl_streaming_runtime.py backend\sdxl_unified_runtime.py backend\staging_manager.py backend\flux_fill_v2\activation.py backend\flux_fill_v2\contracts.py backend\flux_fill_v2\dispatcher.py backend\flux_fill_v2\runtime_state.py backend\flux_fill_v2\t5_posture.py backend\flux_fill_v2\vae_posture.py modules\async_worker.py modules\objr_engine.py modules\pipeline\inference.py modules\pipeline\routes.py modules\pipeline\tiled_refinement.py modules\runtime_surface_state.py modules\runtime_surface_api.py modules\ui_logic.py webui.py tools\check_validation_env.py tracked_tests\test_memory_residency.py tracked_tests\test_pipeline_routes.py tracked_tests\test_pipeline_stage_runtime.py tests\test_runtime_surface_api.py
```
...
```powershell
.\venv\Scripts\python.exe -m pytest tests\test_memory_governor.py tests\test_w11_policy_simplification.py tests\test_async_worker_process_transition.py tests\test_flux_fill_v2.py tests\test_flux_fill_integration.py tests\test_runtime_surface_api.py -q
```
...
- Flux Fill route/session sanity (v2 greenfield and legacy compatibility)
